...
export class JuegoPropioComponent implements OnInit {
  ...
  constructor(
    private alertService: AlertJuegoService,
    ...
    if (this.canva != null && this.cabezaArriba.complete) {
      ...
      for (let i = 1; i < this.snake.length; i++) {
        ...
        this.canva.fillRect(
          ...
          this.snakeSize,
          this.snakeSize
        ); // Dibujar un rectángulo para el cuerpo
      }
    }
  }
  ...
  generarFruta(): void {
    if (this.canvas && this.canvas.nativeElement) {
      const x =
        Math.floor(
          Math.random() * (this.canvas.nativeElement.width / this.snakeSize)
        ) * this.snakeSize;
      const y =
        Math.floor(
          Math.random() * (this.canvas.nativeElement.height / this.snakeSize)
        ) * this.snakeSize;
      this.fruta = { x, y };
    }
  }
